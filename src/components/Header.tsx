...
export const Header: React.FC<HeaderProps> = ({ currentView, onViewChange }) => {
  ...
  const handleCreateAccount = () => {
    onViewChange('signup'); // You'll need to add this view
  };
      ...
      <div className="max-w-4xl mx-auto px-4 py-1">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            {/* Logo completo - visível apenas em telas médias e maiores */}
            <h1 
            ...
            {/* Logo compacto - visível apenas em telas pequenas */}
            <h1 
          ...
          <nav className="hidden md:block">
            {isAuthenticated && user ? (
              ...
              <div className="flex items-center gap-2 text-rixa-cream">
                <FiUser size={20} />
                {/* Botões completos - visíveis em telas médias e maiores */}
          ...
          <MdMenu size={30} color="#FCF6F5FF" className="cursor-pointer md:hidden" />
        </div>
      </div>
    </header>
  );
